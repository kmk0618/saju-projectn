...
function structuredExtras(row: SectionRow) {
  const j = row.content_json || {};
  const basis = Array.isArray(j.key_basis) ? j.key_basis.filter(Boolean) : [];
  const scenes = Array.isArray(j.life_scenes) ? j.life_scenes.filter(Boolean) : [];
  ...
  const table = tableRows.length ? `
    <div class="dataTableWrap">
      <table class="dataTable">
        <tbody>
          ${tableRows.map((r:any) => `<tr><th>${esc(r.label)}</th><td>${esc(r.value)}</td></tr>`).join("")}
        </tbody>
      </table>
    </div>` : "";

  const lifeScenes = scenes.length ? `
    <div class="miniBlock scenes">
      <div class="miniTitle">실제 생활에서는 이렇게 보일 수 있습니다</div>
      <ul>${scenes.map((x:string) => `<li>${esc(x)}</li>`).join("")}</ul>
    </div>` : "";

  const checklistHtml = checklist.length ? `
    <div class="miniBlock checklist">
      <div class="miniTitle">체크 포인트</div>
      <ul>${checklist.map((x:string) => `<li>${esc(x)}</li>`).join("")}</ul>
    </div>` : "";

  const riskAction = (j.risk || j.action_point) ? `
    <div class="twoCol">
      ${j.risk ? `<div class="sideBox"><span>주의할 패턴</span><p>${esc(j.risk)}</p></div>` : ""}
      ${j.action_point ? `<div class="sideBox action"><span>실행 기준</span><p>${esc(j.action_point)}</p></div>` : ""}
    </div>` : "";

  const emphasis = j.emphasis ? `<div class="emphasis">${esc(j.emphasis)}</div>` : "";
  const basisHtml = basis.length ? `<div class="basis"><b>해석 근거</b>${basis.map((x:string) => `<span>${esc(x)}</span>`).join("")}</div>` : "";

  const order = layout === "table" || layout === "timeline"
    ? `${table}${emphasis}${lifeScenes}${riskAction}${checklistHtml}${basisHtml}`
    : layout === "strategy" || layout === "checklist"
      ? `${emphasis}${checklistHtml}${table}${riskAction}${lifeScenes}${basisHtml}`
      : `${emphasis}${table}${lifeScenes}${riskAction}${checklistHtml}${basisHtml}`;
  return order;
}
...
body{font-family:"Noto Sans KR","Malgun Gothic",sans-serif;color:#171717;background:#fff;font-size:10.2pt;line-height:1.82;word-break:keep-all;overflow-wrap:anywhere}
