...
export const PDF_RENDERER_VERSION = "aqua50-editorial-v8-couple-md-locked";
      ...
      <section class="chapterBody">
        ${rows.map((s) => `
          <article class="section layout-${esc(s.content_json?.layout_type || "prose")}" id="section-${s.section_no}">
            ...
            ${reportCategory === "couple" ? "" : editorialSupplement(s)}
          </article>
        `).join("")}
      </section>`;
  }).join("");
...
/* Couple report design lock: preserve the original MD/PDF editorial frame. */
.report-couple .sectionSubtitle{margin:-1mm 0 4mm;padding:0;background:transparent;border:0;border-radius:0;font-size:9.4pt;font-weight:500;line-height:1.7;color:#75631b}
.report-couple .content table{border-collapse:collapse;border-spacing:0;border:0;border-radius:0;overflow:visible;font-size:8.7pt;line-height:1.55}
.report-couple .content th,.report-couple .content td{padding:2.2mm 2.6mm;border-right:0;border-bottom:.2mm solid #ded8cc;text-align:left;vertical-align:top}
.report-couple .content th{background:#2d2a24;color:#fff;font-weight:800}
.report-couple .content tbody tr:nth-child(even) td{background:#f5f2ec}
.report-couple .content blockquote{background:#f2eee5;border-left:1mm solid #c9a962}
.report-couple .content .emph{background:#f2eee5;border:0;border-left:1mm solid #c9a962;border-radius:0;font-weight:600}
.report-couple .supplementCheck,.report-couple .dataTableWrap{display:none!important}
</style>
</head>
<body class="${isCouple ? "report-couple" : isChild ? "report-child" : "report-life"}">
<section class="cover">
