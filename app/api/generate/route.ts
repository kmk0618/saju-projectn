...
async function ensurePdfForReport(sb: any, args: {
  order: any;
  report: any;
  input: any;
}) {
  const reportJson: any = args.report?.report_json || {};
  if (reportJson.pdf_storage_path) {
    return {
      path: reportJson.pdf_storage_path,
      size: reportJson.pdf_size || null,
    };
  }

  const { data: sections, error: sectionsError } = await sb
    .from("report_sections")
    .select("section_no,part_no,part_title,section_title,content_html")
    .eq("report_id", args.report.id)
    .order("section_no", { ascending: true });

  if (sectionsError) throw new Error("PDF_SECTION_LOAD_FAILED:" + sectionsError.message);
  if (!sections || sections.length < REPORT_TOTAL_SECTIONS) {
    throw new Error(`PDF_SECTIONS_INCOMPLETE:${sections?.length || 0}/${REPORT_TOTAL_SECTIONS}`);
  }
  ...
  const html = buildReportHtml({
    title: args.report.title || "종합 인생 리포트",
    subtitle: "132개 질문으로 구성한 개인맞춤 종합 사주 리포트",
  ...
  const storagePath = `guest/${args.order.id}/${args.report.id}.pdf`;
  ...
  const { error: finalError } = await sb.from("reports").update({
    status: "completed",
    generated_at: new Date().toISOString(),
    report_json: finalJson,
    error_message: null,
  }).eq("id", args.report.id);

  if (finalError) throw new Error("PDF_FINALIZE_FAILED:" + finalError.message);

  return { path: storagePath, size: pdfBuffer.length };
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const existingJson: any = report.report_json || {};
    if (report.status === "completed" && existingJson.pdf_storage_path) {
      return J({
        ok:true,
        status:"completed",
        progress:100,
    ...
    if (completed >= REPORT_TOTAL_SECTIONS) {
      const pdf = await ensurePdfForReport(sb, { order, report, input });
      return J({
        ok:true,
        status:"completed",
        progress:100,
    ...
    await sb.from("reports").update({
      ...
      report_json: {
        ...
        progress: done ? 96 : progress,
        phase: done ? "pdf_queued" : phase,
        pdf_ready:false
      },
      generation_model: DEFAULT_MODEL,
      prompt_version: PROMPT_VERSION,
    ...
    if (done) {
      ...
      const pdf = await ensurePdfForReport(sb, { order, report: freshReport, input });
      return J({
        ok:true,
        status:"completed",
        progress:100,
