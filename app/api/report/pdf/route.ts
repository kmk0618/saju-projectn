...
async function loadOrderReport(sb: any, token: string) {
  ...
  const { data: report, error: reportError } = await sb
    .from("reports")
    .select("id,title,status,report_json,created_at,generated_at")
...
async function ensurePdf(sb: any, order: any, report: any) {
  ...
  if (rj.pdf_storage_path) {
    return { path: rj.pdf_storage_path, size: rj.pdf_size || null };
  }
  ...
  if (!sections || sections.length < TOTAL_SECTIONS) {
    throw new Error(`PDF_NOT_READY:${sections?.length || 0}/${TOTAL_SECTIONS}`);
  }

  // Mark PDF stage so the progress page has a truthful status.
  ...
  const path = `guest/${order.id}/${report.id}.pdf`;
...
export async function GET(req: Request) {
  try {
    ...
    // Critical behavior:
    // even if the old generation route already marked the report "completed",
    // if no PDF exists, create it right here on first open/download.
    const pdfInfo = await ensurePdf(sb, order, report);

    const { data: blob, error: fileError } = await sb.storage
      .from("report-pdfs")
      .download(pdfInfo.path);
